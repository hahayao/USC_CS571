...
router.get('/:id', function(req, res){
    ...
    let url = "https://api.themoviedb.org/3/movie/"+movieid+"?api_key=" + apikey + "&language=en-US&page=1";
    ...
    axios.get(url).then(posts=>{
        ...
        tempRumtime = "";
        if(posts.data.runtime != null){
            hour = parseInt(parseInt(posts.data.runtime)/ 60);
            minute = parseInt(posts.data.runtime) % 60;
            tempRumtime = hour + "hrs " + minute + "mins";
        }
        temppost = (posts.data.poster_path?posts.data.poster_path:"");

    finalResults = {
        //youtubePath: resultKey,
        title: (posts.data.title? posts.data.title: ""),
        ...
        release_date: (posts.data.release_date ? posts.data.release_date.slice(0, 4):""),
        runtime: tempRumtime,
        overview: (posts.data.overview?posts.data.overview:""),
        vote_average: (posts.data.vote_average?posts.data.vote_average:""),
